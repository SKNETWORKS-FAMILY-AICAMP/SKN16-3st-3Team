...
|이름|고찰|
| :---: |:---: |
|강동기|LLM과 RAG를 활용해서 무언가 새로운 서비스를 만든다는게 처음에는 어렵게만 느껴져 따라갈 수 있을까 걱정했지만, 팀원들의 도움을 받아 좋은 결과물을 같이 만들 수 있는 시간이였다. 점점 정확도가 올라가는 수치가 직접 보여 뿌듯했고 만든 데이터를 토대로 시각적으로 표현하는 방식에서 어떤 UI/UX가 사용자들이 편안함을 더 느낄 수 있는지 공부할 수 있는 좋은 경험이였다.|
|신지윤|이번 프로젝트는 LLM과 RAG를 활용한 문서 기반 질의응답 시스템이라는 주제로 진행되어 다소 어렵게 느껴졌지만, 실제 구현 과정을 가까이에서 경험하며 많은 것을 배울 수 있었다. 복잡한 데이터 처리와 PDF 추출, 인터페이스 구성 등을 살펴보면서, LLM이 단순 대화형을 넘어 실제 서비스에 적용되는 흐름을 이해할 수 있었고, 이를 통해 앞으로 더 깊이 탐구해야 할 방향도 확인할 수 있었다.|
...
|이현민|평소 Legal AI에 관심이 많았는데 이번 프로젝트를 통해 직접 서비스를 구현할 수 있어 개인적으로 매우 의미 깊었습니다. 법률 데이터를 수집하고 GPT를 통해 문서 전처리 및 청킹 전략을 적용하며 높은 검색 정확도를 끌어올렸지만 AI가 원하는 만큼의 답변을 내놓지 않아 어려움이 있었는데, 이를 해결하기 위해 프롬프트 엔지니어링에 집중하여 민사와 형사를 구분해 상황에 맞는 답변을 제공하는 핵심 로직을 완성할 수 있었습니다. 추후에는 국가법령정보 API를 활용해 최신 판례나 개정사항을 자동으로 수집하고 업데이트하는 모듈을 만들어보고 싶습니다.|
